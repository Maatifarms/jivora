...
export default function AICompanyInsights({ companyName }: InsightsProps) {
  const points = [
    "High supplier response rate guaranteed (< 1 hour)",
    "Trusted by 120+ UK & European wholesale buyers",
    "Fully certified exporter with active HMRC UK customs accords",
    "Direct-from-factory competitive volume pricing tier",
    "99.8% verified on-time manufacturing delivery record"
  ];
    ...
    <div className="rounded-2xl border border-brand-sky-accent/30 bg-brand-sky-light/20 p-6 sm:p-8 space-y-6 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-brand-sky-accent/20 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-base font-extrabold text-navy">AI Factory Audit & Insights</h3>
            <p className="text-[11px] text-navy-muted">Real-time credibility report for {companyName}.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="space-y-3 text-xs text-navy leading-relaxed">
          <p>
            Jivora AI has audited <strong>{companyName}</strong>'s manufacturing credentials, GOTS organic certifications, and past 1,200+ completed export transactions into Northern Europe.
          </p>
          <div className="flex flex-wrap gap-2 pt-1">
            <span className="rounded bg-white px-2.5 py-1 font-bold text-navy border border-grey-border/60">
              ⚡ Guaranteed &lt; 1hr Response
            </span>
        ...
        <div className="rounded-xl bg-white p-4 space-y-3 border border-grey-border/50 shadow-sm">
          <h4 className="text-xs font-extrabold uppercase tracking-wider text-brand-sky-dark flex items-center gap-1.5">
            ...
            <span>Why Jivora Recommends This Supplier:</span>
          </h4>
